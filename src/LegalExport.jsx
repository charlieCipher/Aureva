...
function LegalExport({ assets, userEmail }) {
  ...
  async function generatePDF() {
    ...
    const footerLines = [
      ...
      "Generated by Aureva — Your Secure Legacy Vault",
    ];
    ...
    // Save
    doc.save(`Aureva_Assets_${new Date().toISOString().split("T")[0]}.pdf`);
    setGenerating(false);
  }
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 0.9fr) minmax(280px, 1.1fr)", gap: 24 }}>
        <div>
          ...
          <p style={{ margin: 0, color: "#6f766a", fontSize: 14, lineHeight: 1.6 }}>
            Generate a professional summary of your assets and family instructions.
          </p>
        </div>
        <div>
          <div style={{ background: "#f4f2e9", borderRadius: 18, padding: 16, marginBottom: 16 }}>
            ...
            {[
              ...
              <p key={item} style={{ margin: "0 0 6px 0", color: "#6f766a", fontSize: 13 }}>
                {item}
              </p>
            ))}
          </div>
          ...
          {assets.length === 0 ? (
            <p style={{ color: "#b8554f", fontSize: 13 }}>
            ...
            <button
              ...
              style={{
                width: "100%",
                padding: "14px",
                background: generating ? "#7f9278" : "#171b14",
                color: "#fffefa",
            ...
            >
              {generating ? "Generating PDF..." : "Download Legal Export PDF"}
            </button>
          )}

          <p style={{ margin: "12px 0 0 0", color: "#8a8374", fontSize: 12, lineHeight: 1.5 }}>
            This does not replace a registered legal will. Use alongside proper legal documentation.
          </p>
        </div>
      </div>
    </section>
